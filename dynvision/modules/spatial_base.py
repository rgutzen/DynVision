...
from dynvision.utils.utils import FlatIndices, get_flat_indices
...
class SpatialBase(nn.Module):
    ...
    def _set_default_positions(self) -> Dict[str, LayerPositions]:
        raise NotImplementedError()

    def _get_positions(self, position_dir: Path = None) -> Dict[str, LayerPositions]:
        if position_dir is None:
            positions = {layer: None for layer in self.layer_names}
        else:
            positions = self._load_positions(position_dir)
