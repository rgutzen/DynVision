...
class cordsnet(nn.Module):
    ...
    def __init__(self, dataset, depth):
        ...
        self.inp_conv = nn.utils.parametrizations.weight_norm(
            nn.Conv2d(N_channel, 64, kernel_size=7, stride=2, padding=3, bias=False)
        )
        self.inp_avgpool = nn.AvgPool2d(
            kernel_size=3, stride=2, padding=1, ceil_mode=False
        )
        self.inp_skip = nn.utils.parametrizations.weight_norm(
        ...
        for i in range(self.depth):
            ...
            conv_bias.append(
                nn.Parameter(
                    torch.zeros(channels[i + 1], sizes[i], sizes[i]),
                    requires_grad=True,
                )
            )
            area_area.append(
        ...
        self.out_fc = nn.utils.parametrizations.weight_norm(
            nn.Linear(channels[self.depth], N_class, bias=True)
        )
    ...
    def forward(self, inputs, rank, alpha):
        ...
        for j in range(self.depth):
            rs.append(
                torch.zeros(current_batch_size, channels[j], sizes[j], sizes[j]).to(
                    rank
                )
            )
        ...
        out = self.out_avgpool(
            self.relu(rs[self.depth - 1]) + self.relu(rs[self.depth - 2])
        )
    ...
    def rnn(self, area, r, img, alpha):
        ...
        r[area] = (1 - alpha) * r[area] + alpha * self.relu(
            self.area_conv[area](r[area])
            + self.conv_bias[area]
            + self.area_area[area](areainput)
        )
        return r[area]
    ...
    def record(self, inputs, rank, alpha, layers=[6, 7]):
        ...
        rs = [
            torch.zeros(
                current_batch_size, channels[j], sizes[j], sizes[j], device=rank
            )
            for j in range(self.depth)
        ]
