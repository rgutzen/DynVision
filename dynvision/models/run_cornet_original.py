...
parser.add_argument(
    "--model", choices=["Z", "R", "RT", "S"], default="Z", help="which model to train"
)
parser.add_argument(
    "--times",
    default=5,
...
def set_gpus(n=1):
    ...
    os.environ["CUDA_DEVICE_ORDER"] = (
        "PCI_BUS_ID"  # making sure GPUs are numbered the same way as in nvidia_smi
    )
...
def get_model(pretrained=False):
    ...
    if FLAGS.model.lower() == "r":
        model = model(
            pretrained=pretrained, map_location=map_location, times=FLAGS.times
        )
    else:
...
def train(
    ...
    save_train_epochs=0.1,  # how often save output during training
    save_val_epochs=0.5,  # how often save output during validation
    ...
    if save_train_epochs is not None:
        save_train_steps = (
            np.arange(0, FLAGS.epochs + 1, save_train_epochs) * nsteps
        ).astype(int)
    if save_val_epochs is not None:
        save_val_steps = (
            np.arange(0, FLAGS.epochs + 1, save_val_epochs) * nsteps
        ).astype(int)
    if save_model_epochs is not None:
        save_model_steps = (
            np.arange(0, FLAGS.epochs + 1, save_model_epochs) * nsteps
        ).astype(int)

    results = {
        "meta": {"step_in_epoch": 0, "epoch": start_epoch, "wall_time": time.time()}
    }
        ...
        for step, data in enumerate(tqdm.tqdm(trainer.data_loader, desc=trainer.name)):
            ...
            if FLAGS.output_path is not None:
                ...
                if len(results) > 1:
                    pickle.dump(
                        records,
                        open(os.path.join(FLAGS.output_path, "results.pkl"), "wb"),
                    )
                ...
                if save_model_secs is not None:
                    if time.time() - recent_time > save_model_secs:
                        torch.save(
                            ckpt_data,
                            os.path.join(
                                FLAGS.output_path, "latest_checkpoint.pth.tar"
                            ),
                        )
                        recent_time = time.time()
                ...
                if save_model_steps is not None:
                    if global_step in save_model_steps:
                        torch.save(
                            ckpt_data,
                            os.path.join(
                                FLAGS.output_path, f"epoch_{epoch:02d}.pth.tar"
                            ),
                        )
...
class ImageNetTrain(object):
    ...
    def __init__(self, model):
        ...
        self.lr = torch.optim.lr_scheduler.StepLR(
            self.optimizer, step_size=FLAGS.step_size
        )
